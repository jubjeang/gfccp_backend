...
app.get('/get_permission_access', urlencodedParser, (req, res) => {
    ...
    console.log('get_permission_access user_id: ', req.query['user_id'])
    console.log('get_permission_access RoleId:', req.query['RoleId'])
    dboperations.get_permission_access(req.query['RoleId']).then((result, err) => {
        if (err) {
            console.log('error: ', err)
            res.json({ error: err })
        }
